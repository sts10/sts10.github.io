...
What's a hash and a hash digest? [Here's a good explainer video](https://www.youtube.com/watch?v=b4b8ktEV4Bg), [another video](https://www.youtube.com/watch?v=DMtFhACPnTY), and here's the [Wikipedia page](https://en.wikipedia.org/wiki/SHA-1). But basically a hash is way to represent a piece of data (in this case, a password) without actually revealing it. An example: If we run the word "password" through the SHA-1 hash, we get a "digest" of `5BAA61E4C9B93F3F0682250B6CF8331B7EE68FD8` (which was the 4th most common password in the breach with a whopping 3,645,804 appearances).
