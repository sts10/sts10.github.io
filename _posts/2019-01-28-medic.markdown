...
After the recent breach, I was curious to check my passwords against the list, but I'm a bit paranoid, so I wanted to download the big text file and check my passwords against it offline, nice and safely. 
...
What's a SHA-1 hash? [Here's a good explainer video](https://www.youtube.com/watch?v=b4b8ktEV4Bg), [another video](https://www.youtube.com/watch?v=DMtFhACPnTY), and here's the [Wikipedia page](https://en.wikipedia.org/wiki/SHA-1). Troy Hunt [explains why he uses SHA-1 for this project](https://www.troyhunt.com/introducing-306-million-freely-downloadable-pwned-passwords/#whyhashes):
...
The related, more elegant solution here is Rust's [`if let`](https://doc.rust-lang.org/book/ch06-03-if-let.html), which I use in the rather-convoluted `give_feedback` function, I was happy with the above code as is. Feel free to try implementing `if let` and submit a pull request though! 
