...
The format is `<SHA-1 hash of the password>` and then a colon, then the number of times that particular password appeared in the various breaches (a number I didn't have much interest in). The last 10 lines are very similar, but all the number-of-appearances are `1`

So, to review, our task is to open up a KeePass database, make SHA-1 hashes (digests) of all the passwords, then see if they in this 550-million-line text document. 
...
## Postscript: Collection #2 through #5
