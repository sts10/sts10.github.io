...
I set a strong password (160+ bits).
...
This commend back-ups my Documents and Pictures directories to my S3 bucket.It excludes caches and excludes any files listed in my `restic-excludes.txt` file. Files you might want to exclude are anything very large and/or unimportant.
...
This sent about 7.12Gb to the bucket. Awesome! (Note that I'm still not exactly sure what this will cost me...)
...
In my research and on the fediverse, I saw a few mentions of something called [S3 Glacier](https://docs.aws.amazon.com/glacier/index.html). This does seem like a service that would be good for my use-case. 
...
But then, [elsewhere in the Glacier docs](https://docs.aws.amazon.com/amazonglacier/latest/dev/amazon-glacier-getting-started.html), I spotted this line (bolding mine):
...
I didn't really want to need to install and use the AWS CLI on top of Restic somehow.

This fact, plus not seeing Glacier referred to in the Restic docs gave me pause. For now, I'm sticking with the my standard S3 bucket, which plays nicely with Restic. And I can afford the extra cost.
...
A while back, I wrote a shell wrapper around age and tar that I called [Bottle](https://github.com/sts10/bottle). Given a directory, it `tar`s it, compresses it with [Zstandard](https://facebook.github.io/zstd/), then encrypts the resulting file using [age](https://github.com/FiloSottile/age) using a set key file.

I built this tool to quickly encrypt directories (or files) for my future self. As long as I could access the "bottle" key file, I could decrypt the file and extract the original directory when needed. I envisioned this as handy for archiving things before sending them to less-than-trusted online cloud services.

Partially as an experiment, I Bottle'd a directory of family photos by simply running `bottle family-photos-archives`, then added it to the S3 Restic repo by running: 
```bash
