...
When installing, I had some drive formatting issues pop up. I then used GParted to re-partition the drive to one big ext4 partition. Then in the MX installer, I think I chose the simple partition option, making the entire harddrive `/root`. I waited for the install to complete, then the system rebooted and I was good to go (and remove the live USB). 
