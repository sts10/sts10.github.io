...
Excited, when I went to actually install antiX to this hard drive, I hit a new set of issues that I won't bother laying out here. 

Then I remembered something: In my search for a suitable distro, I tried [MX Linux](https://mxlinux.org/), "a cooperative venture between the antiX and MX Linux communities." The live USB worked well once I simply added `nomodeset` to the GRUB options. But once the GUI booted, I realized why MX Linux is branded as a "middle-weight" distro: it was indeed sluggish. I then switched to trying to install antiX, hopeful that it would work as smoothly as MX, but without the weight. As I learned, for whatever (video-related?) reason, antiX just wasn't easily installing on this machine.

## MX+Fluxbox to the rescue

But! Later I learned that MX officially supports a Fluxbox desktop option. With fingers crossed, I tried installing the 32-bit version and, once I added `nomodeset nosplash` to the GRUB options (bit hitting 'e'), it worked pretty well. Fluxbox+MX turned out to be just lightweight enough to work decently on the iMac. 

When installing, I had some drive formatting issues pop up. 

I then used GParted to re-partition the drive to one big ext4 partition. Then in the MX installer, I think I chose the simple partition option, making the entire harddrive `/root`. (I don't remember if the installer gave me the choice of where to install GRUB, but if it did I almost certainly chose MBR.) 

I then waited for the install to complete, then the system rebooted and I was good to go (and remove the live USB). 

I am still curious why MX worked where antiX did not. Maybe a kernel version difference?
...
I had easier success with antiX and the MacBook. (Here are what I think are [some accurate specs](https://igotoffer.com/apple/macbook-5).)
...
This part below I'm not 100% sure on. Actually, it **may misconfigure your hard drive in such a way that is difficult to recover from**, so I would proceed with lots of **CAUTION**.
...
In the subsequent menus, I chose to, yes, install GRUB and to do it on MBR, which I think is important! (MBR is not the default.)
...
My takeaway here is that antiX is _not_ advertised as a beginner-friendly distro for a reason. (I was relieved that the Bodhi Linux installer didn't ask me any confusing questions!) If you're not paying attention, it can bite ya!
