...
Through [a discussion with a fellow Fediverse user](https://cryptpad.fr/pad/#/2/pad/edit/WcspCeYgcP9ClZNEKPmsdWqK/), I started to think if there was a mathematical way to determine how short the shortest word on a word list could be while maintaining the wonderful entropy-per-word guarantees that lead us to use these word lists to create passphrases in the first place. 
