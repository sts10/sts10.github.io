...
I'm excited to announce that I've add a slew of new features to [Tidy](https://github.com/sts10/tidy), my Rust command-line tool that helps users combine and clean large word lists. 
...
Thus, I added an option in Tidy to enforce a minimum edit distance in the generated word list. I did this by swiping [an `edit_distance` function from this repository of common algorithms written in Rust](https://github.com/TheAlgorithms/Rust/blob/master/src/dynamic_programming/edit_distance.rs) (MIT licensed!). I think it's correct! And it saved me a big headache!
...
I contend that this thought experiment may be useful when we ask what the shortest word on a good word list should be. There may be an established method for determining what this minimum word length should be, but if there is I don't know about it yet. Here's the math I've worked out on my own. 
...
Now we're expecting the word "of" to provide 12.925 (`log2(7776)`) bits of entropy. But when we realize there are only 26 letters in English, we see that, at best, two of these letters only generates `2 * log2(26) == 2 * 4.7 == 9.4` bits of entropy. Again, 12.925 is an overestimate!
...
Running Tidy v 0.2 on the EFF long list with flags `-AA`, Tidy will print:
...
Up for a challenge, I allowed the user to set the number of sides of their dice. This involved some fun non-base-10 work, zero-padding, and a challenging off-by-one error!

```rust
use radix_fmt::*; // Wonder if I can ditch this dependency...
pub fn print_as_dice(n: usize, base: u8, list_length: usize) -> String {
    ...
    // If base is a common dice size, we'll add 1 to every 
    // digit, to make it easier to compare to rolled dice
...
However, these diceware lists are often a set length so they use every possible dice roll (6^5 == 7,776 for EFF long list, or 6^4 == 1,294 words for their short lists). Thus I also needed to implement a way to cut lists to a set length.
...
Users can do this with the `-c`/`--cut-to` flag.
...
```bash
tidy -P -x 4 --cut-to 7776 --dice 6 1password-2021.txt
```
